...
With resiliency of the network a key concern to the DSO and frequently mentioned as a requirement for the smart grid in the literature, FLISR is one such technique which has been
...
The report specifies that an essential component for successful FLISR operations is the communications networks used to monitor and enable control over switching systems and devices, in combination with suitable interfaces which allow
...
\\
\\
A paper by \textit{Eriksson et al.} \cite{7000578} further discuss the limitations and considerations when implementing FLISR solutions, specifically centralised deployments, such as having a single point of failure and the requirement for the handling of large amounts of real-time data, large network topologies and the processing necessary to ensure consistent operation. They propose a
...
these switches communicate a Fault Passage Indicator (FPI) and Loss of Voltage Indicator and in this way it is possible to identify a fault occurrence. ESB Networks notes that a number of their networks currently rely upon
